  ...
  async handler(event, context) {
    try {
      ...
      if (process.env.GITHUB_ACCESS_TOKEN) {
        /** Changes for multiple branches in GitHub begin **/
        const repos = [`GitHubPagesRebuild`, `GitHubPagesStore`, `GitHubPagesRebuild`];
        for (repo of repos) {
        /** Changes for multiple branches in GitHub end **/
          const response = await fetch(
            `https://api.github.com/repos/beggsn/${repo}/actions/workflows/taskit.yml/runs`,
            {
              headers: {
          ...
          if (response.ok) {
            ...
            if (
              ...
              const rebuild = await fetch(
                `https://api.github.com/repos/beggsn/${repo}/actions/workflows/taskit.yml/dispatches`,
                {
                  method: "POST",
                  headers: {
                    Accept: "application/vnd.github.v3+json",
